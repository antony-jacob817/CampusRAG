...
/**
 * Stream edited query response via SSE Fetch
 */
export const streamEditChatMessage = async ({
  messageId,
  text,
  department = 'all',
  signal,
    ...
    while (true) {
      ...
      for (const block of lines) {
        ...
        try {
          ...
          console.warn('[SSE Edit] JSON parse error on chunk:', parseErr, dataStr);
        }
      }
    }
  } catch (err) {
    if (err.name === 'AbortError') {
      console.log('[SSE Edit] Stream aborted by user.');
      return;
    }
    if (onError) {
      onError(err);
    } else {
      console.error('[SSE Edit Streaming Error]', err);
    }
  }
};
